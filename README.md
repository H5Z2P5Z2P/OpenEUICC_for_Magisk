# [OpenEUICC repository URL](https://gitea.angry.im/PeterCxy/OpenEUICC)
...
KernelSU 3.x users must install a metamodule that provides systemless mounts, such as `meta-overlayfs`, before installing this module. The GitHub Release includes the tested `meta-overlayfs` ZIP for convenience. OpenEUICC must be scanned from `system_ext/priv-app` as a privileged app; installing it as a normal `/data/app` package will not grant the required privileged permissions.
