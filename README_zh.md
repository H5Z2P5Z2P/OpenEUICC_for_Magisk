# [OpenEUICC源码存储库网址](https://gitea.angry.im/PeterCxy/OpenEUICC)
...
KernelSU 3.x 用户需要先安装提供 systemless 挂载能力的 metamodule，例如 `meta-overlayfs`，再安装本模块。GitHub Release 会附带本次验证使用的 `meta-overlayfs` ZIP，方便一起下载。OpenEUICC 必须作为 `system_ext/priv-app` 下的特权应用被系统扫描；如果被安装成普通 `/data/app`，系统不会授予所需的特权权限，可能会出现闪退或打不开。
